/* ============================================================================
 * validate-audio-batch.mjs — 第 01 批德语音频文件校验脚本
 *
 * 用法：
 *   node scripts/validate-audio-batch.mjs 01                        # 校验完整批次 01
 *   node scripts/validate-audio-batch.mjs 01 --limit=10             # 部分校验前10张
 *   node scripts/validate-audio-batch.mjs 01 --from=DE-0001 --to=DE-0010
 *   node scripts/validate-audio-batch.mjs 01 --json                 # 输出 JSON
 *
 * 校验规则：
 *   1. 每张卡片必须有 word 音频文件
 *   2. 每张卡片必须有 example 音频文件
 *   3. 不检查 meaning 音频
 *   4. 文件大小不能为 0
 *   5. 文件扩展名为 .mp3
 *   6. manifest textHash 与当前词卡文本对比
 *   7. 检测意外中文文件
 * ============================================================================ */
...
function parseArgs(argv) {
  const args = { batch: '01', limit: null, from: null, to: null, json: false };
...
function loadManifest() {
  const manifestPath = join(ROOT, 'data', 'audio-manifest-batch-01.json');
...
function validateAudio(allCards, manifest, batch, cliArgs) {
  ...
  // 检查意外中文文件 — 仅针对当前检查范围内的卡片
  checkChineseAudioFiles(results, checkedIds);

  // pass 判定：所有预期德语 mp3 都存在且大小正常
...
function printReport(results) {
  ...
  if (results.partial && results.pass) {
    console.log('  [INFO] 部分校验通过。完整第 01 批需检查所有 325 张卡片。');
  }
  console.log('');
...
function main() {
  ...
  // 校验
  const results = validateAudio(allCards, manifest, batch, cliArgs);
  ...
  writeFileSync(
    join(reportsDir, 'audio-validation-batch-01.json'),
