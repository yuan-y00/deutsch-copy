/* ============================================================================
 * generate-audio-batch02.mjs — 第 02 批 A1 德语音频生成脚本
 *
 * 用法：
 *   node scripts/generate-audio-batch02.mjs                                    # 完整生成 (默认 provider)
 *   node scripts/generate-audio-batch02.mjs --provider=piper --limit=10 --force  # Piper 前10张
 *   node scripts/generate-audio-batch02.mjs --provider=piper --from=DE-0326 --to=DE-0335
 *   node scripts/generate-audio-batch02.mjs --dry-run                          # 仅生成任务清单
 *   node scripts/generate-audio-batch02.mjs --card DE-0350                     # 仅生成单张卡
 *
 * Provider 选择（按优先级）：
 ...
 *
 * 输出：
 *   audio/de/words/DE-0326.mp3 ... DE-0650.mp3
 *   audio/de/examples/DE-0326.mp3 ... DE-0650.mp3
 *   data/audio-manifest-batch-02.json
 * ============================================================================ */
...
function printReport(report) {
  ...
  console.log('\n═══════════════════════════════════════');
  console.log(`  第 02 批 A1 德语音频生成报告 ${label}`);
  console.log('═══════════════════════════════════════');
  ...
  if (report.provider === 'dry-run') {
    ...
    console.log('    node scripts/generate-audio-batch02.mjs --provider=piper --limit=10 --force');
  }

  if (report.partial) {
    console.log('  [INFO] 这是部分生成。完整第 02 批需处理全部 325 张卡片。');
  }
...
async function main() {
  ...
  console.log(`[Deutsch Copy] 第 02 批 A1 德语音频生成`);
  ...
  let cards = loadCards();
  if (!cards || cards.length === 0) {
    console.error('[ERROR] 无法加载第 02 批词卡数据');
    process.exit(1);
  }
  ...
  const manifestPath = join(Dirs.data, 'audio-manifest-batch-02.json');
  ...
  writeFileSync(
    join(reportDir, 'audio-generation-batch-02.json'),
